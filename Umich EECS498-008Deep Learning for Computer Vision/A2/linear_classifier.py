...
def svm_loss_vectorized(
    ...
    margins[torch.arange(num_train), y] = 0.0  # correct_class 不参与计算
    # 4.loss计算
    loss = torch.max(margins, torch.zeros_like(margins)).sum()
    loss /= num_train
    loss += reg * torch.sum(W * W)
    ...
    # Replace "pass" statement with your code
    # 5.Gradient
    mask = (margins > 0).double()  # (N, C)
    row_sum = mask.sum(dim=1)    # 对每个样本 i，统计它产生了多少个正margin

    # 对正确类别所在的列写入负的计数（-row_sum），这一步把
    # “-X_i” 加到正确类别的梯度里
    mask[torch.arange(X.shape[0]), y] = -row_sum
    # margins[torch.arange(num_train, device=X.device), y] = -row_sum
    # 最终的梯度 = Xᵀ @ mask / N  + 2·reg·W
    dW = X.t().mm(mask) / X.shape[0] + 2 * reg * W
...
def sample_batch(
    ...
    indices = torch.randint(low=0, high=num_train,
                            size=(batch_size,), device=X.device)
    X_batch = X[indices]
    y_batch = y[indices]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def train_linear_classifier(  # NOTE: SGD
    loss_func: Callable,
    W: torch.Tensor,
    X: torch.Tensor,
    y: torch.Tensor,
    ...
    for it in range(num_iters):
        ...
        if type(loss) == type(torch.tensor([1])):
...
def test_one_param_set(
    ...
    cls.train(  # train model in X_train
        X_train=X_train,
        y_train=y_train,
        learning_rate=lr,
        reg=reg,
    ...
    train_acc = (cls.predict(X_train) == y_train).double().mean().item()
    val_acc = (cls.predict(X_val) == y_val).double().mean().item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    # here, it is easy to run into numeric instability (Check NOTE:Numeric      #
    # Stability in http://cs231n.github.io/linear-classify/). Plus, don't       #
    # forget the regularization!                                                #
    #############################################################################
    # Replace "pass" statement with your code
    N = X.shape[0]
    C = W.shape[1]
    ...
    for i in range(N):
        ...
        loss += (- torch.log(exp_scores[y[i]] / sum_exp_scores))

        for j in range(C):
            # softmax pr
            p = exp_scores[j] / sum_exp_scores

            # FIXME
            # 对于正确类别 y[i]，梯度是 (p - 1)
            # 对于其余类别，梯度是 p
            if j == y[i]:
                dW[:, j] += (p - 1.0) * X[i]      # (D,) -> (D, 1)
            else:
                dW[:, j] += p * X[i]
...
def softmax_loss_vectorized(
    ...
    # Store the loss in loss and the gradient in dW. If you are not careful     #
    # here, it is easy to run into numeric instability (Check NOTE: Numeric     #
    # Stability in http://cs231n.github.io/linear-classify/). Don't forget the  #
    # regularization!                                                           #
    #############################################################################
    # Replace "pass" statement with your code
    scores = X@W
    ...
    pr = exp_scores / sum_exp_scores

    # loss
    pr_correct_class = pr[torch.arange(X.shape[0]), y]
    loss = (- torch.log(pr_correct_class)).mean()  # mean of all loss

    loss += reg * torch.sum(W * W)

    # FIXME: computer the gradient
    # Subtract 1 from the probabilities of the correct classes
    pr[torch.arange(X.shape[0]), y] -= 1.0
    dW = X.t().mm(pr)
    dW /= X.shape[0]  # Normalization
