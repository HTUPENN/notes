...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    # NOTE: As a practice, we will NOT allow to use torch.relu and torch.nn ops
    ...
    hidden[hidden <= 0] = 0.0
    # print('after', hidden)
    # hidden = torch.clamp(hidden, min=0)  # ReLU activation
...
def nn_forward_backward(
    ...
    pr_correct_class = pr[torch.arange(N), y]
    loss = - torch.log(pr_correct_class).mean()
    ...
    dpr[torch.arange(N), y] -= 1
    dpr /= N  # Average over the batch

    # Compute the gradient w.r.t. W2 and b2
    grads['W2'] = h1.T @ dpr + 2 * reg * W2
    grads['b2'] = dpr.sum(dim=0)  # Sum over the batch for bias gradients

    # Compute the gradient w.r.t. W1 and b1
    dh1 = dpr @ W2.T
    dh1[h1 <= 0] = 0  # Backprop ReLU
...
def nn_train(
    ...
    - NOTE: learning_rate_decay: Scalar giving factor used to decay the learning rate
      after each epoch.
    ...
    for it in range(num_iters):
        ...
        # Replace "pass" statement with your code
        for para in params:
            params[para] -= learning_rate * grads[para]
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
...
def find_best_net(
    ...
    for lr, hidden_size, reg_strength, lr_decay in param_combinations:
        # instane
        net = TwoLayerNet(input_size=data_dict["X_train"].shape[1], hidden_size=hidden_size, output_size=len(
            set(data_dict['y_train'])))
        # train
...
def find_best_net(
    data_dict: Dict[str, torch.Tensor], combos: List, current_idx: int = 0, best_val_acc: float = 0.0
):
    """
    分类版本
    """
    ...
    for i in range(current_idx, len(combos)):
        lr, hidden_size, reg_strength, lr_decay = combos[i]
        # instane
        net = TwoLayerNet(input_size=data_dict["X_train"].shape[1], hidden_size=hidden_size, output_size=len(
            set(data_dict['y_train'])))
        # train
        ...
        if val_acc > best_val_acc:
            ...
            # best_index_in = i
        print(
            f"one combo training done. current idx in combos : {i}, current best_val_acc : {best_val_acc} ")
