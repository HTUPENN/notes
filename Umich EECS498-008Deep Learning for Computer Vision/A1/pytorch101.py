...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    # Replace "pass" statement with your code
    last_row = x[-1, :].clone()
    third_col = x[:, 2:3].clone()
    first_two_rows_three_cols = x[:2, :3].clone()
    even_rows_odd_cols = x[::2, 1::2].clone()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    out = (
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = x[list(range(x.shape[0]-1, -1, -1))]
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    max_row = len(x)
    max_column = max(x) + 1
    y = torch.zeros(max_row, max_column)
    y[list(range(max_row)), x] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    y.reshape(2, -1)
    y.reshape(2, 3, 4)
    y.transpose(0, 1)  # or y.permute(1, 0, 2)
    y.reshape(3, 8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    x[list(range(x.shape[0])), row_min_idx] = 0
    y = x
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def mm_on_gpu(x: Tensor, w: Tensor) -> Tensor:
    ...
    x_gpu = x.cuda()
    w_gpu = w.cuda()
    y = torch.mm(x_gpu, w_gpu)   # shape (A, C), 仍在 GPU 上
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    padded = torch.pad_sequence(xs, batch_first=True, padding_value=0)
    row_sum = padded.sum(dim=1)
    y = row_sum / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:  # 比较相邻的元素
    """
    ...
    """
    # --------------------------------------------------------------
    # 1️⃣ 对 x 进行 **稳定排序**，得到排序后的数值以及对应的原始下标
    # --------------------------------------------------------------
    # sorted_vals : 按值升序排列的结果（shape = (N,))
    # perm        : 排序前的下标，即 sorted_vals[i] 在原始 x 中的位置
    # 注意：stable=True 确保相同数值的相对顺序不变
    sorted_vals, perm = torch.sort(x, stable=True)

    # --------------------------------------------------------------
    # 2️⃣ 生成一个布尔掩码，标记每个“新出现的数值”
    #    第一个元素必然是新值，随后只要当前值 != 前一个值即为新值
    # --------------------------------------------------------------
    N = x.shape[0]
    mask = torch.ones(N, dtype=torch.bool)   # 全部 True
    if N > 1:                     # 当 N==1 时下面的切片会报错，直接保留全 True 即可
        mask[1:] = sorted_vals[1:] != sorted_vals[:-1]

    # --------------------------------------------------------------
    # 3️⃣ 通过掩码一次性取出唯一值与对应的“首次出现下标”
    # --------------------------------------------------------------
    uniques = sorted_vals[mask]   # shape (K,)
    indices = perm[mask]          # 对应的原始下标 (K,)
